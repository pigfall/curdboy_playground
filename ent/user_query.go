...
func (uq *UserQuery) QueryDept() *DeptQuery {
	query := &DeptQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
	...
	if query := uq.withDept; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.DeptPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "dept": %w`, err)
		}
		query.Where(dept.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "dept" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Dept = append(nodes[i].Edges.Dept, n)
			}
		}
	}
